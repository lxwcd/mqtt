...
##### 客户端使用自签名证书  
测试自签名证书未连接成功，用 mqttx 和 mqtt.fx 测试。  
服务端将客户端自签名证书的路径添加到 `cacertfile` 配置项中，需要确定是否支持客户端自签名证书。  
...
在网络传输中，TCP 是面向字节流的协议，先发送 `0x12`，然后发送 `0x34`。这符合大端字节序的要求。  
...
- 两者都定义QoS不同的情况下  
如果发布者和订阅者定义的QoS级别不同，MQTT broker（消息中介）会根据两者定义的QoS级别选择一个合适的QoS级别传递给订阅者。具体来说，MQTT broker会选择发布者和订阅者QoS级别中的较低者。  
...
如果 Client 在发送 QoS 1 的消息后没有在预定的超时时间内收到 PUBACK 数据包，那么它会再次尝试发送消息。这个过程可以重复多次，直到接收到 PUBACK 确认为止。  
...
如果发布者（Publisher）在发送消息后没有在预定的超时时间内收到PUBACK数据包，那么它会再次尝试发送消息。这个过程可以重复多次，直到接收到PUBACK确认为止。  
...
1. 发送方重传机制  
当发送方（Publisher）在发送QoS 2的PUBLISH消息时，如果在超时时间内没有收到PUBREC消息，它会将PUBLISH消息的DUP标志设置为1，并重新发送该消息。这种重传机制是为了确保消息能够成功到达接收方。  
2. 接收方的处理机制  
接收方（Subscriber）在收到PUBLISH消息后，会立即回复一个PUBREC消息，无论PUBLISH消息是否是重传的。接收方会记录PUBLISH消息的Packet Identifier（PID），并将其保存在本地。对于重复的PUBLISH消息，接收方会忽略它们，因为它们的PID已经被记录，且接收方知道这些是重复的消息。  
3. 消息去重机制  
接收方在收到PUBREL消息后，会正式将消息传递给上层应用，并销毁该PID。之后，无论发送方发送多少个PUBREL消息，接收方都不会再处理重复的PUBLISH消息，因为这些消息的PID已经被销毁。  
4. 总结  
发送方：在收到PUBREC之前，可以重传PUBLISH消息。一旦收到PUBREC并发送PUBREL，发送方就进入Packet ID释放流程，不能再使用当前PID重传PUBLISH消息。  
接收方：在收到PUBREL之前，会忽略重复的PUBLISH消息。一旦收到PUBREL，接收方会销毁PID，并将消息传递给上层应用。  
  
这种机制确保了即使发送方重传了多次PUBLISH消息，接收方也不会重复处理这些消息，从而保证了消息的唯一性和可靠性。  
...
4. 结论  
在 MQTT 中，消息的超时重传机制是由 MQTT 协议本身实现的，而不是由 TCP 层实现的。TCP 层的重传机制确保了数据包的传输可靠性，而 MQTT 协议的重传机制确保了消息的 QoS 级别。   
...
5. **使用命令行工具**：对于某些 broker，如 Mosquitto，可以使用命令行工具来管理消息，包括删除保留消息。  
